...
*   **作者與單位**：G. Astolfi, L. Barboni, F. Cocchioni, C. Pepe and S. M. Zanoli @ Università Politecnica delle Marche & i.Process S.r.l., Italy
*   **論文網址**：Not Provided
*   **原始碼/實作**：Not Provided (論文提及為專有軟體工具 Proprietary software tool)
...
*   **工程優化建議**：
    *   **砍掉過時模組**：若要在現有機台落地，建議徹底簡化其繁雜的「黑盒子系統識別（Black-box identification）」與「LPV 線性化降階」流程。
    *   **技術升級**：考慮導入 **非線性模型預測控制（NMPC）** 或是基於深度學習的 **NN-MPC** 框架，直接將物理方程式轉化為可微分的深度學習算子。此外，必須引入多模態感測融合（例如整合排煙溫度與爐壁熱像儀），打破單一出口測溫儀的單點故障風險，以提升工業現場的環境魯棒性與可移植性。
